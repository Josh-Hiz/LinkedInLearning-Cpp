...
namespace bw {
    ...
    typedef uint64_t numnum;    // uint64_t type = numnum
    ...
    public:
        numword() : _num(0) {} //Default case, sets num = 0
        numword(const numnum num) : _num(num) {} //Makes whatever the argument equal to the current _num
        ~numword(); //Destructor
        ...
        const char * operator () ( const numnum num ) { return words(num); }; //The functor that's actually called in main()
    };
